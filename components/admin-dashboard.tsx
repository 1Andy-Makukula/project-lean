                                ...
                                {intents.map((intent) => (
                                    <tr key={intent.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="py-4 text-right whitespace-nowrap">
                                            <IntentAction intent={intent} isPending={isPending} successId={successId} onMarkPaid={handleMarkPaid} />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
...
function ShopsTab() {
    ...
    function handleCreate() {
        ...
        startTransition(async () => {
            const result = await createShop(name.trim(), location.trim(), pinCode.trim());
            if (result.success) {
                setName(""); setLocation(""); setPinCode(""); setShowForm(false);
                ...
                <Card className="border-slate-200">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-slate-500">Price (MWK) *</label>
...
function IntentAction({ intent, isPending, successId, onMarkPaid }: {
    intent: AdminIntent; isPending: boolean; successId: string | null; onMarkPaid: (id: string) => void;
}) {
    if (successId === intent.id) {
        return <span className="inline-flex items-center gap-1 text-xs font-semibold text-green-600"><CheckCircle2 className="h-3.5 w-3.5" />Done</span>;
    }
    if (intent.status === "payment_submitted") {
        return (
            <div className="flex items-center justify-end gap-3">
                {intent.paymentReference && (
                    <code className="rounded bg-slate-100 px-2 py-1 font-mono text-xs font-semibold text-slate-600">
                        {intent.paymentReference}
                    </code>
                )}
                <Button className="h-8 gap-1.5 px-3 text-xs bg-brand-gradient text-white shadow-sm hover:brightness-110 transition-all" onClick={() => onMarkPaid(intent.id)} disabled={isPending}>
                    <ShieldCheck className="h-3.5 w-3.5" />{isPending ? "Updating…" : "Mark as Paid"}
                </Button>
            </div>
        );
    }
